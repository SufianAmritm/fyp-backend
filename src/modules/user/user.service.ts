...
import { DOMAIN_ENTITY, RESPONSE_MESSAGES, Roles } from 'src/common/constants';
...
export class UserService implements IUserService {
  ...
  async createUser(
    ...
    try {
      ...
      const adminRoleId =roles.find(role=>role.name===Roles.ADMIN).id;
      const employeeRoleId =roles.find(role=>role.name===Roles.EMPLOYEE).id;
      if(admin){
        userMap.roleId=adminRoleId;
        userMap.emailVerified=true
        userMap.password = await this.utilService.hash(password);
      ...
      const emailData={
        ...
        frontendBaseUrl: this.configService.get('FRONTEND_BASE_URL'),
      }
      if(!admin){
  ...
  async findOneByEmail(email: string): Promise<User> {
    const findOptions = new FindOptionsBuilder<User>().where({email}).relations({
      role:true
    }).build()
  ...
  async findOneById(id: number): Promise<User> {
    const findOptions = new FindOptionsBuilder<User>().where({id}).relations({
      role:true
    }).build()
    const user = await this.userRepository.findOneWithBuilderOption(
     findOptions
    );
